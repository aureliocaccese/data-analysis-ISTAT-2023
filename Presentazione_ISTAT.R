...
mod_bi <- glmer(povassc~comp + cond+ sp_tot_str_aggr_2 +(1+sp_tot_str_aggr_2|rgn),family=binomial, data=data, weights = w_anno)
summary(mod_bi)
...
mod_norm = lm(logsp~ as.factor(pnasc_1) + c_etacalc_1 + c_titstu_1   + D179_bf + Risecon + Sitecon + povassc, data = data, weights = w_anno)
